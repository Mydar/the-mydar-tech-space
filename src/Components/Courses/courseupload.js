...
function CourseUpload(props) {
    ...
    const handleSubmit = (event) => {
        ...
        if (title === "" && photoUrl === "" && videoUrl === "" && description === "" && Track === "") {
    ...
    const handleUpload = () => {
        Axios.post("https://myjsondb.herokuapp.com/Courses", { ...input })
        .then(res => {
    ...
    const uploadFile = (file, shouldSaveURLToPhoto = true, loaded, total) => {
        ...
        console.log(`fileuploadprogress data.loaded: ${loaded}, data.total: ${total}`)
        }
    ...
    const handleFiles = (event) => {
        ...
        let shouldSaveURLToPhoto;
        if (event.target.name === "coverimage") {
            shouldSaveURLToPhoto = true
        } else if (event.target.name === "video") {
            shouldSaveURLToPhoto = false
        }
        uploadFile(file, shouldSaveURLToPhoto, loaded, total)
            ...
            <div className="mt-3">
                <form onSubmit={handleSubmit} method="POST">
                        ...
                        <div className="form-group">
                            ...
                            <div>
                                <div className="img-div mb-2"  style={{width: 200, height: 200}}>
                                    <img src={input.photoUrl} alt={input.title} />
                                </div>
                            <input name="coverimage" type="file" accept="image/*" onChange={handleFiles} required/>
                        ...
                        <div className="form-group">
                            ...
                            <div>
                                <input name="video" type="file" accept="video/*" onChange={handleFiles} required/>
                            </div>
                        </div>
